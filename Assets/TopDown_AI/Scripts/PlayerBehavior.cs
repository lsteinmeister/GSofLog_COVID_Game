...
public class PlayerBehavior : MonoBehaviour {
	...
	public int HP;
	public List<Image> HP_Icons;
	public Sprite Health_full, Health_empty;
	...
	//	float speedY = inputVertical > 0.1 ? Mathf.Clamp ((inputVertical * moveSpeed), moveSpeed / 2.0f, moveSpeed) : 0.0f;
		...
		if(transform.position.y > 1.5 || transform.position.y<0.1f) 
		{
			Debug.Log("Player Height needs adjusrting");
	...
	public void DamagePlayer(){
		...
		HP -=1;
		HP_Icons[HP].sprite= Health_empty;
		if(HP <= 0) Death();		
	}
	public void Death(){
	...
	public void Attack(){
		switch (currentWeapon) {
			...
			GameCamera.ToggleShake (0.1f);
				...
				bullet.transform.LookAt(new Vector3(mousePointer.transform.position.x,0.5f, mousePointer.transform.position.z));
	...
	void AlertEnemies(){
		RaycastHit[] hits=Physics.SphereCastAll (hitTestPivot.position,12.0f, hitTestPivot.up);
	...
	public void Heal()
	{
		...
		HP_Icons[HP].sprite= Health_full;	
		HP +=1;
	}
